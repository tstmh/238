...
public class PAMessageReceiver implements MessageListener {
	
private static final Logger logger = LoggerFactory.getLogger(PAMessageReceiver.class);
	...
	public void onMessage(Message pMessage) {
		if (logger.isInfoEnabled()) {
			logger.info("PA MessageReceiver started receiving the message.....");
		}
		try {
			if (pMessage instanceof ObjectMessage) {
				...
				if (logger.isInfoEnabled()) {
					logger.info("PA lMsgId received :: " + lMsgId);
				}
