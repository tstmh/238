...
public class WMCSMessageReceiver implements MessageListener {
	
private static final Logger logger = LoggerFactory.getLogger(WMCSMessageReceiver.class);
	...
	public void onMessage(Message pMessage) {
		if (logger.isInfoEnabled()) {
			logger.info("WMCSMessageReceiver started receiving the message.....");
		}
		try {
			if (pMessage instanceof ObjectMessage) {
				...
				if (MessageConstants.TECH_ALARM_ID.equals(lMsgId)) {
					...
					tunnelMessageHandler.handleWMCSStatus((EquipStatusDtoList)objectMessage.getObject());
				} else if (MessageConstants.TUNNEL_MEASURE_ID.equals(lMsgId)) {
					tunnelMessageHandler.handleWMCSMeasure((MeasureDetailDtoList)objectMessage.getObject());
				}
			}
