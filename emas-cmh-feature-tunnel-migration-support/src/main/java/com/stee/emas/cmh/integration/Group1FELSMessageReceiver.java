...
public class Group1FELSMessageReceiver implements MessageListener {
	...
	public void onMessage(Message pMessage) {
		if (logger.isInfoEnabled()) {
			logger.info("Group1JmsListener started receiving the message.....");
		}
		try {
			if (pMessage instanceof ObjectMessage) {
				...
				if (MessageConstants.LUS_MW_STATUS_ID.equals(lMsgId)) {
					group1MessageHandler.handleLUSMWStatus((LusStatusDto)objectMessage.getObject());
				} else if (MessageConstants.LUS_LINK_STATUS_ID.equals(lMsgId)) {
					group1MessageHandler.handleLUSLinkStatus((LusStatusDto)objectMessage.getObject());
				} else if (MessageConstants.LUS_STATUS_ID.equals(lMsgId)) {
					group1MessageHandler.handleLUSStatus((LusStatusDto)objectMessage.getObject());
				} else if (MessageConstants.FELS_STATUS.equals(lMsgId)) {
					group1MessageHandler.handleFelsStatus((FelsApplicationStatusDto)objectMessage.getObject());
				} else if (MessageConstants.FIRE_STATUS_ID.equals(lMsgId)) {
					group1MessageHandler.handleFireStatus((FireStatusDto)objectMessage.getObject());
				}
			}
