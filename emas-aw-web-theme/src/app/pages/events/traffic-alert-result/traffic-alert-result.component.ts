...
@Component({
  selector: 'emas-traffic-alert-result',
  templateUrl: './traffic-alert-result.component.html',
  styleUrls: ['./traffic-alert-result.component.css']
})
export class TrafficAlertResultComponent implements OnInit {
	...
	submitForm(): void {
		...
		if (equipId && expwayCode && this.alertListByValue == 1) { // equipID
			...
			this.eventsService.getAIVideoByEquipIdAndExpwayCode(parameter).subscribe((r) => {

				const res: any = r;
				const resData = res.Body.getAIVideoByEquipIdAndExpwayCodeResponse.aiVideoList;
			...
			expwayCode ? this.eventsService.getAIVideoByAlertTypeAndExpwayCode(parameter).subscribe((r) => {

				const res: any = r;
				const resData = res.Body.getAIVideoByAlertTypeAndExpwayCodeResponse.aiVideoList;
				const handeData = resData ? [...resData] : []; // 防止没有数据时后台返回undefined
				this.mergeData(handeData);
			...
			expwayCode ? this.eventsService.getAIVideoByAlertTimeAndExpwayCode(parameter).subscribe((r) => {

				const res: any = r;
				const resData = res.Body.getAIVideoByAlertTimeAndExpwayCodeResponse.aiVideoList;
				const handeData = resData ? [...resData] : []; // 防止没有数据时后台返回undefined
				this.mergeData(handeData);
				this.listOfData = resData ? [...resData] : [];
				this.listOfData.length ? this.message.create('success', `Data request succeeded`) : this.message.create('warning', `No Data`);
	...
	async getAllData() {
		...
		this.eventsService.getAIVideoByEquipIdAndExpwayCode(parameter).subscribe((r) => {
			...
			this.mergeData(handeData);
			this.listOfData = resData ? [...resData] : []; // 防止没有数据时后台返回undefined
			this.listOfData.length ? this.message.create('success', `Data request succeeded`) : this.message.create('warning', `Please select filter criteria`);
			const ttt: number = this.listOfData.length / 10;
			const yu = this.listOfData.length % 10;
	...
	getDataByTime(transferFromDate, transferToDate) {
		...
		this.eventsService.getAIVideoByAlertTimeAndExpwayCode(parameter).subscribe((r) => {
